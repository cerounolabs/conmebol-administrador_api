...
/*MODULO PERMISOS*/
    ...
    $app->post('/v2/200/testpcr', function($request) {
        ...
        $val05      = $request->getParsedBody()['testpcr_solicitante_nombre'];
        $val06      = $request->getParsedBody()['testpcr_solicitante_apellido'];
        $val07      = $request->getParsedBody()['testpcr_solicitante_documento'];
        $val08      = $request->getParsedBody()['testpcr_solicitante_email'];
        $val09      = $request->getParsedBody()['testpcr_solicitante_observacion'];
        $val10      = $request->getParsedBody()['testpcr_jefetura_documento'];
        $val11      = $request->getParsedBody()['testpcr_fecha_1'];
        $val12      = $request->getParsedBody()['testpcr_fecha_2'];
        $val13      = $request->getParsedBody()['testpcr_hora_1'];
        $val14      = $request->getParsedBody()['testpcr_hora_2'];
        $val15      = $request->getParsedBody()['testpcr_adjunto_1'];
        $val16      = $request->getParsedBody()['testpcr_adjunto_2'];
        $val17      = $request->getParsedBody()['testpcr_adjunto_3'];
        $val18      = $request->getParsedBody()['testpcr_adjunto_4'];
        $val19      = $request->getParsedBody()['testpcr_laboratorio_nombre'];
        $val20      = $request->getParsedBody()['testpcr_laboratorio_contacto'];
        $val21      = $request->getParsedBody()['testpcr_laboratorio_email'];
        $val22      = $request->getParsedBody()['testpcr_laboratorio_fecha_resultado'];
        $val23      = $request->getParsedBody()['testpcr_laboratorio_adjunto'];
        $val24      = $request->getParsedBody()['testpcr_laboratorio_resultado'];
        $val25      = $request->getParsedBody()['testpcr_laboratorio_observacion'];
        $val26      = $request->getParsedBody()['testpcr_carga_usuario'];
        $val27      = $request->getParsedBody()['testpcr_carga_fecha'];
        $val28      = $request->getParsedBody()['testpcr_carga_ip'];
        $val29      = $request->getParsedBody()['testpcr_talento_usuario'];
        ...
        if (isset($val01) && isset($val02) && isset($val05) && isset($val06) && isset($val07)) {
            $sql00  = "INSERT INTO [hum].[SOLPCR](                                                                          SOLPCREST, SOLPCRORD,                                                                                  SOLPCRTSC,                                                                          SOLPCRTRC, SOLPCRNOM, SOLPCRAPE, SOLPCRDOC, SOLPCRDOJ, SOLPCREMA, SOLPCRFE1, SOLPCRFE2, SOLPCRHO1, SOLPCRHO2, SOLPCRAD1, SOLPCRAD2, SOLPCRAD3, SOLPCRAD4, SOLPCRUSC, SOLPCRFEC, SOLPCRIPC, SOLPCROBC, SOLPCRAUS, SOLPCRAFH, SOLPCRAIP) 
                                        SELECT (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'TESTPCRESTADO' AND DOMFICPAR = ?),          ?, (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'TESTPCRSOLICITUD' AND DOMFICPAR = ?), (SELECT DOMFICCOD FROM adm.DOMFIC WHERE DOMFICVAL = 'TESTPCRROL' AND DOMFICPAR = ?),        ?,         ?,         ?,          ?,        ?,          ?,        ?,          ?,         ?,        ?,         ?,          ?,        ?,         ?, GETDATE(),         ?,         ?,         ?, GETDATE(),       ?
                                                                                                                                $val01, $val04, $val02, $val03, $val05, $val06, $val07, $val10, $val08, $val11, $val12, $val13, $val14, $val15, $val16, $val17, $val18, $val26, $val28, $val09, $aud01, $aud03, $val01, $val02, $val07, $val11
