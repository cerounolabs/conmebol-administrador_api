    ...
    $app->get('/v1/000/marcacion', function($request) {
        ...
        try {
            ...
            while ($rowMSSQL = $stmtMSSQL->fetch()) {
                ...
                $marcacion_salida_almuerzo_hora = (strlen(trim($rowMSSQL['marcacion_salida_almuerzo_hora'])) > 3 ? date("H:i", strtotime(trim($rowMSSQL['marcacion_entrada_oficina_hora']))): date("H:i", strtotime('0'.trim($rowMSSQL['marcacion_salida_almuerzo_hora']))));
                $marcacion_entrada_almuerzo_hora= (strlen(trim($rowMSSQL['marcacion_entrada_almuerzo_hora'])) > 3 ? date("H:i", strtotime(trim($rowMSSQL['marcacion_entrada_oficina_hora']))) : date("H:i", strtotime('0'.trim($rowMSSQL['marcacion_entrada_almuerzo_hora']))));
                $marcacion_salida_oficina_hora  = (strlen(trim($rowMSSQL['marcacion_salida_oficina_hora'])) > 3 ? date("H:i", strtotime(trim($rowMSSQL['marcacion_entrada_oficina_hora']))) : date("H:i", strtotime('0'.trim($rowMSSQL['marcacion_salida_oficina_hora']))));
